...
class GenerationService:
    ...
    async def generate(
        ...
        voice_prefix = "en"
        subtitles_path = subtitles.generate_subtitles(
            audio_path=tts_path,
            sentences=sentences,
            audio_clips=paths,
            voice=voice_prefix,
        ...
        combined_video_path = clip.combine_videos(
            video_paths, temp_audio.duration, n_threads or 2
        )
        final_video_path = clip.generate_video(
            combined_video_path,
            tts_path,
            subtitles_path,
            n_threads or 2,
    ...
    async def add_generation(
        self, createGenerationDTO: CreateGenerationSchema, db: AsyncSession
    ):
        """
    ...
    async def get_user_generations_from_upload_id(
        self, upload_id: uuid.UUID, db: AsyncSession
    ):
        """
    ...
    async def get_user_quiz_from_upload_id(
        self, upload_id: uuid.UUID, db: AsyncSession
    ):
        """
    ...
    async def get_user_uploads_with_generations(
        ...
        for upload in uploads:
            ...
            results.append(
                [[upload.id, upload.project_name, upload.file_path], generations, quiz]
            )

        return results

    async def update_upload_project_name(
        self, upload_id: uuid.UUID, new_project_name: str, db: AsyncSession
    ):
        """
