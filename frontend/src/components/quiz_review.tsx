...
import { ArrowLeft, ArrowRight, CheckCircle, XCircle, Trophy, RotateCcw, Home, BookOpen, Target } from "lucide-react"
...
const sampleUserAnswers: UserAnswer[] = [
  ...
  { questionId: 2, selectedAnswer: 0, isCorrect: false }, // Selected Venus instead of Mars
  { questionId: 3, selectedAnswer: 1, isCorrect: true }, // Correctly selected Blue Whale
  { questionId: 4, selectedAnswer: 2, isCorrect: false }, // Selected 1946 instead of 1945
...
export default function QuizReview() {
  ...
  const correctAnswers = sampleUserAnswers.filter((answer) => answer.isCorrect).length
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-xl mb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-800">All Questions Summary</h1>
                  <p className="text-gray-600">Complete overview of your quiz performance</p>
                </div>
              </div>
          ...
          <div className="space-y-6">
            {sampleQuestions.map((question, index) => {
              ...
              return (
                <div key={question.id} className="bg-white rounded-3xl p-6 shadow-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                    {question.options.map((option, optionIndex) => {
                      ...
                      return (
                        <div key={optionIndex} className={`p-3 rounded-xl border-2 ${getStatusStyles(status)}`}>
                          <div className="flex items-center justify-between">
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-200 via-pink-100 to-purple-300 py-8 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl mb-6">
            ...
            <div className="flex items-center space-x-3">
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold ${
                  currentUserAnswer.isCorrect ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                }`}
              >
                {currentQuestionIndex + 1}
              </div>
              <div>
                <div className="font-semibold text-gray-800">Question {currentQuestionIndex + 1}</div>
                <div className="flex items-center space-x-2">
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${
                  ...
                  {currentUserAnswer.isCorrect ? (
                    <span className="text-xs px-2 py-1 rounded-full bg-green-100 text-green-700">Correct</span>
                  ) : (
                    <span className="text-xs px-2 py-1 rounded-full bg-red-100 text-red-700">Incorrect</span>
                  )}
                </div>
              </div>
            </div>
          ...
          {/* Question */}
          <div className="mb-8">
            <h2 className="text-2xl font-semibold text-gray-800 leading-relaxed">{currentQuestion.question}</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {currentQuestion.options.map((option, index) => {
              ...
              return (
                <div key={index} className={`p-4 rounded-2xl border-2 ${getStatusStyles(status)}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div
                  ...
                  {status === "incorrect-selected" && <div className="mt-2 text-sm text-red-600">Your answer</div>}
                  {status === "correct" && index !== currentUserAnswer.selectedAnswer && (
                    <div className="mt-2 text-sm text-green-600">Correct answer</div>
                  )}
                </div>
              )
            })}
